      ...
      {radky.length === 0 ? (
        ...
        <div className="mt-8 grid gap-8 md:grid-cols-[minmax(0,1fr)_300px]">
          <div className="grid gap-4">
            {radky.map(({ klic, produkt, velikost, pocet }) => (
              <div key={klic} className="grid grid-cols-[88px_minmax(0,1fr)] gap-4 rounded-lg border bg-card p-4 shadow-card">
                <img
                  src={produkt.obrazek}
                  alt={produkt.nazev}
                  width={900}
                ...
                <div className="min-w-0">
                  ...
                  {velikost && (
                    <p className="text-sm font-semibold text-primary">Velikost rámu {velikost}</p>
                  )}
                  <p className="text-sm text-muted-foreground">{formatCena(produkt.cena)}</p>
                  <div className="mt-3 flex items-center gap-2">
                    <Button variant="outline" size="icon" onClick={() => zmenit(klic, pocet - 1)} aria-label="Ubrat">
                      <Minus className="h-4 w-4" />
                    </Button>
                    <span className="w-8 text-center font-semibold">{pocet}</span>
                    <Button variant="outline" size="icon" onClick={() => zmenit(klic, pocet + 1)} aria-label="Přidat">
                      <Plus className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => odebrat(klic)} aria-label="Odebrat">
